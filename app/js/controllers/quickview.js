 ...
 * 
 */
angular.module('sywStyleXApp').controller('QuickViewCtrl', 
		['$rootScope', '$scope', '$state','$stateParams', '$timeout', 'ngDialog', 'UtilityService', 'CartService', 'localStorageService', 'ProductDetailService',  function($rootScope, $scope, $state, $stateParams, $timeout, ngDialog, UtilityService, CartService, localStorageService, ProductDetailService){
			var addToCartLocker = false;
			  ...
			  $scope.isLikesShown=false;
			  $scope.isRelatedPostShown=false;
			  ...
			  var initializeAttr = function() {
				  ...
				  var imageSizeInitialize=function(){
					  var windowWidth=$(".container").width();
					  $(".images").width(windowWidth*0.6);
...
//				    $scope.addToCartDisabled = false;
				  ...
				  var getCurrentProductDetails = function() {
				    ...
				    productDetailLocker = true;
				     ...
				     ProductDetailService.getProductDetail($scope.ngDialogData.productId).then(function(response){
...
//				    var visualTag = {id: products.visualTagId};
				  ...
				  $scope.toggleDescription = function() {
				    if ($scope.isDescriptionShown) {
				      ...
				      $scope.onclicklabeldescription={"opacity":"0.4","border-bottom":"none"};
				    } else {
				      $scope.isDescriptionShown = true;
				      $scope.isLikesShown=false;
				      $scope.isRelatedPostShown=false;
				      $scope.onclicklabeldescription={"opacity":"1","border-bottom":" 1px solid #4ac0d1"};
				      $scope.onclicklabellikes={"opacity":"0.4","border-bottom":"none"};
				      $scope.onclicklabelrelatedpost={"opacity":"0.4","border-bottom":"none"};
				    }
				  };
				  $scope.toggleLikes=function(){
					  if($scope.isLikesShown){
						  $scope.isLikesShown=false;
						   $scope.onclicklabellikes={"opacity":"0.4","border-bottom":"none"};
					  }else{
						  $scope.isLikesShown=true;
						  $scope.isDescriptionShown=false;
						  $scope.isRelatedPostShown=false;
						  $scope.onclicklabellikes={"opacity":"1","border-bottom":" 1px solid #4ac0d1"};
						  $scope.onclicklabeldescription={"opacity":"0.4","border-bottom":"none"};
						  $scope.onclicklabelrelatedpost={"opacity":"0.4","border-bottom":"none"};
					  }
				  };
				  $scope.toggleRelatedPost=function(){
					  if($scope.isRelatedPostShown){
						  $scope.isRelatedPostShown=false;
						  $scope.onclicklabelrelatedpost={"opacity":"0.4","border-bottom":"none"};
					  }else{
						  $scope.isRelatedPostShown=true;
						  $scope.isLikesShown=false;
						  $scope.isDescriptionShown=false;
						  $scope.onclicklabelrelatedpost={"opacity":"1","border-bottom":" 1px solid #4ac0d1"};
						  $scope.onclicklabeldescription={"opacity":"0.4","border-bottom":"none"};
						  $scope.onclicklabellikes={"opacity":"0.4","border-bottom":"none"};
					  }
				  }
